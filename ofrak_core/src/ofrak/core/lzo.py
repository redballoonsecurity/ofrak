...
class LzoUnpacker(Unpacker[None]):
    """
    Decompresses LZO (Lempel-Ziv-Oberhumer) compressed data. Use when analyzing firmware or software that uses LZO compression, often identifiable by specific magic bytes. The fast decompression makes it popular for real-time compression needs.
    """
...
class LzoPacker(Packer[None]):
    """
    Compresses data using LZO (Lempel-Ziv-Oberhumer) algorithm. Use after modifying decompressed LZO data to recreate compressed sections in firmware.
    """
