...
class AddCommentModifier(Modifier[AddCommentModifierConfig]):
    """
    Adds analysis comments to resources for documentation purposes, attaching text annotations to
    specific resources or byte ranges. Comments can document findings, mark important locations,
    ...
    and help track analysis progress and findings.
    """
...
class DeleteCommentModifier(Modifier[DeleteCommentModifierConfig]):
    """
    Removes one or more comments from resources based on range, content, or removes all comments.
    Can target specific comments by text or location, or clear all comments from a resource. Use for
    cleaning up outdated annotations, removing incorrect analysis notes, clearing comment clutter,
    updating documentation by removing old comments, or resetting comment state. Helps maintain
    clean analysis state by removing no-longer-relevant documentation.
    """
