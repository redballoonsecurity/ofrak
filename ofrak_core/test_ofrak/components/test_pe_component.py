...
def pe_root_resource(ofrak_context: OFRAKContext, request) -> Resource:
    return _get_test_resource_from_file_name(ofrak_context, request.param)


@pytest.fixture(params=["kernel32.dll", "x64kernel32.dll"])
def kernel32_dll(ofrak_context: OFRAKContext, request) -> Resource:
    return _get_test_resource_from_file_name(ofrak_context, request.param)


def test_pe_unpacker(pe_root_resource: Resource):
    pe_root_resource.unpack_recursively()

    pe_view = pe_root_resource.view_as(Pe)
    optional_header = pe_view.get_optional_header()
    assert isinstance(optional_header, PeOptionalHeader)

    # Get the sections, tet get_header / get_flags()
    sections: List[PeSection] = list(pe_view.get_sections())
    ...
    code_regions = list(pe_view.get_sections())
    assert code_regions is not []
    code_region = pe_view.get_section_by_name(".text")
    code_region_header = code_region.get_header()
    assert code_region_header.name == ".text"
    assert code_region_header.get_body() == code_region


def test_pe_get_memory_region_for_vaddr(kernel32_dll: Resource):
    """
    Test that Program.get_memory_region_for_vaddr works for Pe as intended.
    """
    kernel32_dll.unpack()
    pe_view = kernel32_dll.view_as(Pe)
    for section in pe_view.get_sections():
        section_header = section.get_header()
        virtual_address = section_header.m_virtual_address
        memory_region = pe_view.get_memory_region_for_vaddr(virtual_address + 4)
        assert memory_region.virtual_address == virtual_address


def _get_test_resource_from_file_name(ofrak_context: OFRAKContext, test_file_name: str) -> Resource:
