...
@pytest.fixture
def executable_resource(ofrak_context, elf_executable_file) -> Resource:
    root_resource = ofrak_context.create_root_resource_from_file(elf_executable_file)
...
def test_adding_comments(executable_resource: Resource):
    """Test adding comments, including to the entire resource (range=None)."""
    executable_resource.run(
        AddCommentModifier,
...
def test_adding_comments_same_range(executable_resource: Resource):
...
def test_comment_content(executable_resource: Resource, comment_str: str):
...
def test_range_validation(executable_resource: Resource):
    """Test that adding comments to out-of-bounds ranges raises an error."""
    data_len = len(executable_resource.get_data())
...
def test_comments_survive_unpacking(executable_resource: Resource, recursively: bool):
    ...
    # Unpack the resource
    if recursively:
        component_run_result = executable_resource.unpack_recursively()
    else:
        component_run_result = executable_resource.unpack()
...
@pytest.mark.parametrize("recursively", [True, False])
def test_comments_survive_repacking(executable_resource: Resource, recursively: bool):
    ...
    if recursively:
        executable_resource.unpack_recursively()
        executable_resource.pack_recursively()
    else:
        executable_resource.unpack()
        executable_resource.pack()
    comment_attributes = executable_resource.get_attributes(CommentsAttributes)
    assert get_comment_count(comment_attributes) == 1


def test_comments_survive_analyzing(executable_resource: Resource):
    ...
    executable_resource.analyze(Magic)
    comment_attributes = executable_resource.get_attributes(CommentsAttributes)
    assert get_comment_count(comment_attributes) == 1


def test_deleting_comments(executable_resource: Resource):
...
def test_deleting_non_existing_comment(executable_resource: Resource):
