...
class TestJffs2UnpackModifyPack(UnpackModifyPackPattern):
    ...
    def unpack(self, jffs2_resource: Resource) -> None:
        jffs2_resource.unpack_recursively()

    def modify(self, unpacked_zip_resource: Resource) -> None:
        jffs2_v = unpacked_zip_resource.view_as(Jffs2Filesystem)
        child_text_string_config = StringPatchingConfig(6, "ofrak")
        child_textfile = jffs2_v.get_entry(JFFS2_ENTRY_NAME)
        child_textfile.resource.run(StringPatchingModifier, child_text_string_config)

    def repack(self, jffs2_resource: Resource) -> None:
