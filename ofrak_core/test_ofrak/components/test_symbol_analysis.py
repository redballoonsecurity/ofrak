...
def binja_components(ofrak_injector):
...
async def test_elf_symbolic_analysis(
    unstripped_elf_resource: Resource,
    expected_symbols: Dict[str, Tuple[int, ElfSymbolType]],
):
    await unstripped_elf_resource.unpack_recursively(do_not_unpack=(CodeRegion,))
    ...
    for sym_name, (expected_vaddr, expected_type) in expected_symbols.items():
        ...
        assert expected_type == analyzed_sym.symbol_type

    assert "puts" not in analyzed_syms_by_name
...
async def test_complex_block_symbolic_analysis(
    ...
    assert "puts" in analyzed_syms_by_name


async def test_all_symbolic_analysis(
    unstripped_elf_resource: Resource,
    expected_symbols: Dict[str, Tuple[int, ElfSymbolType]],
):
    await unstripped_elf_resource.unpack_recursively()
    lb = await unstripped_elf_resource.view_as(LinkableBinary)

    analyzed_syms = await lb.get_symbols()
    analyzed_syms_by_name: Dict[str, LinkableSymbol] = {sym.name: sym for sym in analyzed_syms}
    ...
    foo_sym = analyzed_syms_by_name["foo"]
    foo_sym.resource.add_view(dataclasses.replace(foo_sym, virtual_address=-1))
    await foo_sym.resource.save()

    with pytest.raises(ValueError) as e:
        await lb.get_symbols()
