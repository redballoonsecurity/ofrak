...
class TestIhexUnpackPack(UnpackModifyPackPattern):
    ...
    def modify(self, unpacked_root_resource: Resource) -> None:
        ...
        child_to_modify = next(iter(unpacked_root_resource.get_descendants(r_filter=r_filter)))

        child_to_modify.queue_patch(Range(0, len(self.REPLACEMENT_STRING)), self.REPLACEMENT_STRING)
        child_to_modify.save()
    ...
    def verify(self, repacked_root_resource: Resource) -> None:
        ...
        results = repacked_root_resource.search_data(search_term)
        assert len(results) == 1

        # Should be able to unpack again at the end
        repacked_root_resource.unpack_recursively()


@pytest.mark.skipif_missing_deps([IhexPacker, IhexUnpacker])
@pytest.mark.parametrize("ihex_file", IHEX_TEST_FILES)
def test_ihex_analyzer(ofrak_context: OFRAKContext, ihex_file):
    from bincopy import BinFile

    root = ofrak_context.create_root_resource_from_file(ihex_file)
    raw_ihex_data = root.get_data()
    binfile = BinFile()
    binfile.add_ihex(raw_ihex_data.decode("utf-8"))

    ihex_prog_data = binfile.as_binary()

    ihex_program_child = root.create_child(
        tags=(IhexProgram,),
        data=ihex_prog_data,
    )

    ihex_prog = ihex_program_child.view_as(IhexProgram)
