...
def test_patch_from_source_modifier(
    ofrak_context: OFRAKContext,
    large_elf_file,
    patch_file,
) -> None:
    def add_and_return_segment(resource: Resource, vaddr: int, size: int) -> ElfProgramHeader:
        ...
        resource.run(LiefAddSegmentModifier, config)
        ...
        file_segments = resource.get_descendants_as_view(
        ...
        code_region.resource = resource.create_child_from_view(
    ...
    def call_new_segment_instead(root_resource: Resource, new_segment: ElfProgramHeader):
        ...
        call_instruction = main_cb.resource.get_only_descendant_as_view(
            v_type=Instruction,
            r_filter=ResourceFilter(
        ...
        call_instruction.modify_assembly("call", f"0x{new_segment.p_vaddr:x}")

    def apply_patch(resource: Resource, source_dir: str, new_segment: ElfProgramHeader):
    ...
    resource = ofrak_context.create_root_resource_from_file(large_elf_file)
    new_segment = add_and_return_segment(resource, 0x440000, 0x2000)

    output_file_name = os.path.join(os.path.dirname(patch_file), "test_patch")

    source_dir = os.path.join(os.path.dirname(patch_file))

    apply_patch(resource, source_dir, new_segment)
    call_new_segment_instead(resource, new_segment)

    resource.pack()
    resource.flush_data_to_disk(output_file_name)
