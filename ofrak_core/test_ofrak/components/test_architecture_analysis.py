...
def test_architecture_analysis(resource: Resource):
    """
    ...
    first_memory_region_resource = next(
        resource.get_children(ResourceFilter.with_tags(MemoryRegion))
    )
    program_attributes_from_memory_region = first_memory_region_resource.analyze(ProgramAttributes)
