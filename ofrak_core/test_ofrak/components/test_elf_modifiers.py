...
def test_elf_add_symbols(ofrak_context: OFRAKContext, elf_executable_file, elf_test_directory):
    ...
    original_elf.run(ElfAddStringModifier, ElfAddStringModifierConfig(strings_to_add))

    output_path = os.path.join(elf_test_directory, "program_with_newstrings")
    original_elf.flush_data_to_disk(output_path)
...
def test_elf_force_relocation(ofrak_context: OFRAKContext, elf_object_file, elf_test_directory):
    # Modify this object file so main calls `bar` instead of `foo`
    # This causes the process to exit with code 24 instead of 12
    original_elf = ofrak_context.create_root_resource_from_file(elf_object_file)
    original_elf.unpack()

    elf = original_elf.view_as(Elf)
    ...
    elf.resource.flush_data_to_disk(os.path.join(elf_test_directory, "program_relocated.o"))
...
def test_modifier(
    ...
    assert len(views) > 0
    for view in views:
        for entry in view.get_entries():
            entry.resource.run(modifier, modifier_config)
    mod_path = elf_executable_file + "_mod"
    elf.resource.flush_data_to_disk(mod_path)
    mod_elf = ofrak_context.create_root_resource_from_file(mod_path)
    mod_elf.unpack()
    views = list(
        mod_elf.get_children_as_view(
            test_view,
            ResourceFilter(tags=(test_view,)),
        )
    )
...
@pytest.fixture
def elf_resource(elf_executable_file: str, ofrak_context: OFRAKContext):
    return ofrak_context.create_root_resource_from_file(elf_executable_file)


def test_elf_program_header_modifier(elf_resource: Resource):
    """
    ...
    for program_header in elf.get_program_headers():
        ...
        program_header.resource.run(
            ElfProgramHeaderModifier, ElfProgramHeaderModifierConfig(0x1337)
        )
        updated_program_header = program_header.resource.view_as(ElfProgramHeader)
...
class TestElfPointerArraySectionModifier:
    ...
    def test_elf_pointer_array_section_modifier(self, elf_resource: Resource):
        """
        Test that `ElfPointerArraySectionModifier` modifies the underlying pointer bytes.
        """
        pointer_array_section = self._unpack_and_get_first_pointer_array_section(elf_resource)
        original_data_values = list()
        for entry in pointer_array_section.get_entries():
            original_data = entry.resource.get_data()
            original_data_values.append(original_data)

        pointer_array_section.resource.run(
            ElfPointerArraySectionAddModifier,
            ElfPointerArraySectionAddModifierConfig(skip_list=(), add_value=self.add_value),
        )
        updated_pointer_array_section = pointer_array_section.resource.view_as(
            ElfPointerArraySection
        )

        for i, entry in enumerate(updated_pointer_array_section.get_entries()):
            new_data = entry.resource.get_data()
            assert new_data != original_data_values[i]

    def _unpack_and_get_first_pointer_array_section(self, elf_resource):
        elf_resource.unpack()
        pointer_array_section = list(
            elf_resource.get_descendants_as_view(
...
@pytest.fixture
def hello_out(ofrak_context: OFRAKContext) -> Resource:
    return ofrak_context.create_root_resource("hello.out", hello_elf())


def test_lief_add_segment_modifier(hello_out: Resource, tmp_path):
    """
    ...
    hello_out.flush_data_to_disk(original_path)
    ...
    hello_out.run(LiefAddSegmentModifier, config)

    # Assert new segment is in extended binary
    extended_path = tmp_path / "extended"
    hello_out.flush_data_to_disk(extended_path)
...
def test_lief_add_section_modifier(hello_out: Resource, tmp_path):
    config = LiefAddSectionModifierConfig(name=".test", content=b"test", flags=0)
    hello_out.run(LiefAddSectionModifer, config=config)
    elf_path = tmp_path / "test.elf"
    hello_out.flush_data_to_disk(elf_path)
    assert segment_exists(elf_path, ".test", content=b"test")


def test_lief_remove_section_modifier(hello_out: Resource, tmp_path):
    original = tmp_path / "original.elf"
    hello_out.flush_data_to_disk(original)
    assert segment_exists(original, ".text")
    config = LiefRemoveSectionModifierConfig(name=".text")
    hello_out.run(LiefRemoveSectionModifier, config=config)
    modified = tmp_path / "modified.elf"
    hello_out.flush_data_to_disk(modified)
