...
@pytest.fixture
def elf_permstest(elf_permstest_executable_file: str, ofrak_context: OFRAKContext) -> Resource:
    """
    An ELF with all combinations of permission flags
    """
    return ofrak_context.create_root_resource_from_file(elf_permstest_executable_file)


def test_elf_perms_header_analysis(elf_permstest: Resource):
    """
    Test getting memory permissions for all flag combinations
    """
    elf_permstest.unpack()
    elf = elf_permstest.view_as(Elf)
    for prog_header in elf.get_program_headers():
