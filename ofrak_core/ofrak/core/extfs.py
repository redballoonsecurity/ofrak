...
from ofrak import Unpacker, Resource, ResourceAttributes
...
from ofrak.core.filesystem import FilesystemEntry, SymbolicLink
...
class ExtUnpacker(Unpacker[None]):
    ...
    async def unpack(self, resource: Resource, config: ComponentConfig = None) -> None:
        with tempfile.NamedTemporaryFile(suffix=".extfs") as temp_fs_file:
            ...
            resource.add_attributes(await ExtAnalyzer._analyze_path(temp_fs_file.name))
...
class ExtPacker(Packer[None]):
    ...
    async def mke2fs(cls, resource: Resource, path: str):
        ...
        if int(reserved_blocks_pct * ext_fs_attrs.block_count / 100):
        ...
        if need_tune2fs_reserved_block:
            ...
            if proc.returncode:
                raise CalledProcessError(
                    returncode=proc.returncode, cmd=tune2fs_cmd, output=stdout, stderr=stderr
                )

    @classmethod
    async def _flush_to_disk_and_generate_debugfs_script(
        cls, root: ExtFilesystem, data_dir: str, script: _DebugfsScript
    ) -> str:
        ...
        async for parent, _, folders, files in root.walk():
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(script, "w") as script_file:
                await self._flush_to_disk_and_generate_debugfs_script(
                    filesystem, data_dir, _DebugfsScript(script_file)
                )
            ...
            if proc.returncode:
                raise CalledProcessError(
                    returncode=proc.returncode, cmd=debugfs_cmd, output=stdout, stderr=stderr
                )
            ...
            if orig_fs_size > len(new_fs_data):
                resource.queue_patch(
                    Range(len(new_fs_data), orig_fs_size), b"\0" * (orig_fs_size - len(new_fs_data))
                )
