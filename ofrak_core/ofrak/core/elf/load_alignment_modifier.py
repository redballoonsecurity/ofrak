...
class ElfLoadAlignmentModifier(Modifier[None]):
    ...
    def modify(self, resource: Resource, config=None) -> None:
        """
        ...
        """
        elf: Elf = resource.view_as(Elf)
        load_program_headers = list(get_load_program_headers(elf))
        ...
        for first_program_header, second_program_header in _pairwise(
            ...
            trimmed_potential_free_space = self._trim_potential_free_space_by_vaddr(
            ...
            first_program_header.resource.run(
                ElfProgramHeaderModifier,
                ElfProgramHeaderModifierConfig(
    ...
    @staticmethod
    def _trim_potential_free_space_by_vaddr(
...
def get_load_program_headers(elf: Elf) -> List[ElfProgramHeader]:
...
def main(ofrak_context: OFRAKContext, file: str, output_file: str):
    root_resource = ofrak_context.create_root_resource_from_file(file)
    root_resource.identify()
    root_resource.run(ElfUnpacker)
    root_resource.run(ElfLoadAlignmentModifier)
    if root_resource.has_tag(Allocatable):
        allocatable = root_resource.view_as(Allocatable)
        ...
        root_resource.flush_data_to_disk(output_file)
