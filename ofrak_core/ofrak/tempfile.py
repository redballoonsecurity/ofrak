"""
Copied from cpython/main/Lib/tempfile.py in Python 3.12
Modified NamedTemporaryFile so that delete_on_close is by default False 
Modified TemporaryDirectory._rmtree call to shutil.rmtree to use backward
compatible parameter onerror instead of onexc
...
def NamedTemporaryFile(
    ...
    *,
    ...
    'delete_on_close' -- if 'delete', whether the file is deleted on close
       (default False) or otherwise either on context manager exit
...
class SpooledTemporaryFile(_io.IOBase):
    ...
    def __del__(self):
        if not self.closed:
            _warnings.warn(f"Unclosed file {self!r}", ResourceWarning, stacklevel=2, source=self)
            self.close()
...
class TemporaryDirectory:
    ...
    def __init__(
        self, suffix=None, prefix=None, dir=None, ignore_cleanup_errors=False, *, delete=True
    ):
        ...
        def onerror(func, path, exc_info):
            if isinstance(exc_info[0], PermissionError):
                ...
                try:
                    ...
                    pass
            elif isinstance(exc_info[0], FileNotFoundError):
                pass
            else:
