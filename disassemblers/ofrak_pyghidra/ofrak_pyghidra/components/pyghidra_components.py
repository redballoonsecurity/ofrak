...
class PyGhidraUnpacker(Unpacker[None]):
    ...
    async def unpack(self, resource: Resource, config: Optional[PyGhidraUnpackerConfig] = None):
        ...
        with TemporaryDirectory() as tempdir:
            ...
            with pyghidra.open_program(program_file) as flat_api:
                ...
                backend_code_regions = sorted(ghidra_code_regions,
                                              key=lambda cr: cr.virtual_address)
                ...
                for code_region in ofrak_code_regions:
                    ...
                    for backend_code_region in backend_code_regions:
                        backend_relative_va = (
                                ...
                                backend_code_regions[0].virtual_address
                        )
                        if backend_relative_va == relative_va and backend_code_region.size == code_region.size:
                            code_region.virtual_address = backend_code_region.virtual_address
                            break

                await asyncio.gather(
                    *[code_region.resource.save() for code_region in ofrak_code_regions]
                )

                for code_region in ofrak_code_regions:
                    func_cbs = await self.unpack_code_region(code_region, program_attributes, flat_api)
    ...
    async def unpack_code_region(self, code_region: CodeRegion, program_attributes, flat_api):
        functions = []
        start_address = flat_api.getAddressFactory().getDefaultAddressSpace().getAddress(
            hex(code_region.virtual_address)
        )
        end_address = flat_api.getAddressFactory().getDefaultAddressSpace().getAddress(
            hex(code_region.virtual_address + code_region.size)
        )
    ...
    async def unpack_complex_block(
        ...
        for block in bb_iter:
            ...
            while block is not None:
                ...
                if (
                    _parse_offset(successor_bb_address_range.getMinAddress())
                    >= _parse_offset(func.getBody().getMinAddress())
                    and _parse_offset(successor_bb_address_range.getMaxAddress())
                    <= _parse_offset(func.getBody().getMaxAddress())
                ):
                    is_exit_point = False
                    if (
                        exit_vaddr is None
