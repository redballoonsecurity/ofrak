...
def add_and_return_segment(elf_resource: Resource, vaddr: int, size: int) -> ElfProgramHeader:
    ...
    elf_resource.run(LiefAddSegmentModifier, config)
    elf_resource.unpack_recursively()

    # Get our newly added segment. First get all ElfProgramHeaders, then return the one
    # with our virtual address.
    file_segments = elf_resource.get_descendants_as_view(
    ...
    code_region.resource = elf_resource.create_child_from_view(
...
def call_new_segment_instead(resource: Resource, new_segment: ElfProgramHeader):
    """Replace the original `call` instruction in main with a call to the start of `new_segment`."""
    main_cb = resource.get_only_descendant_as_view(
        v_type=ComplexBlock,
    ...
    call_instruction.modify_assembly("call", f"0x{ghidra_new_segment_vaddr:x}")


def patch_uppercase(resource: Resource, source_dir: str, new_segment: ElfProgramHeader):
...
def main(ofrak_context: OFRAKContext, file_path: str, output_file_name: str):
    ...
    new_segment = add_and_return_segment(root_resource, 0x108000, 0x2000)
    source_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "src", "example_7"))
    patch_uppercase(root_resource, source_dir, new_segment)
    call_new_segment_instead(root_resource, new_segment)
